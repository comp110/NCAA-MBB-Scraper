...
class TarHeelTraditionKey extends BasketballAlgo {
  ...
  Scoreline kButter(Team away, Team home) {
    return new Scoreline("KButter Factor", away.getAssistTurnoverRatio() * 10, home.getAssistTurnoverRatio() * 10);
  }

  Scoreline makeItWayne(Team away, Team home) {
    return new Scoreline("Make it Wayne", away.getThreesAttempted() / away.getGamesPlayed(),
        home.getThreesAttempted() / home.getGamesPlayed());
  }

  Scoreline hensoned(Team away, Team home) {
    return new Scoreline("Hensoned", (away.getAvgBlocks() - home.getAvgBlocks()) * 10,
  ...
  Scoreline ballDontLie(Team away, Team home) {
    ...
    } else if (home.getFtPercent() > away.getFtPercent()) {
  ...
  Scoreline reallyBigTeam(Team away, Team home) {
    return new Scoreline("Really Big Team", away.getRoster().size(), home.getRoster().size());
  }
